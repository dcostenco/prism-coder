...
- Fourteenth and fifteenth rounds: the history screen has an aggregate classifier-call budget as a safety net at the structural maximum (170 uncached calls per request; every shape the plan caps allow fits under it, so a paid call never trips it), beyond which it fails closed as UNCERTAIN with the attempt named; and a consecutive-ERROR breaker: after three windows in a row answer ERROR (a dead or stalled classifier), the rest are marked ERROR without a call and the ERROR path's keyword net decides, fail-closed, instead of minutes of timeouts. An explicit empty `messages: []` is single-turn for the prompt cap too.
