...
- **History screened alone, the request in context.** Every history turn is classified alone
  ...
  single-prompt error always took: cloud when allowed, otherwise the keyword
  net over the whole conversation decides, and three errors in a row are
  treated as uncertain. Then the current prompt is classified in
  context (the tail of the role-labelled transcript ending with it), which can
  only raise the verdict. The deterministic rules run per turn (role-aware for
  the operational ones), the keyword floor and reserved-category attribution
  over the whole conversation. A reserved phrase in any turn, the user's or the
  model's own earlier answer, refuses the call exactly as it would in a single
  prompt.
...
### Hardened by twenty-two adversarial review rounds before merge
...
- Twenty-second round (two verifier reports): every verdict from a turn read alone is now kept — OBVIOUS_RESERVED final, UNCERTAIN fail-closed (cloud when the plan allows it, else refused), ERROR on the single-prompt error path as before — and the only context read is the current prompt's own window, which can raise the verdict but never clear one. Four shapes of "an UNCERTAIN read alone defers to context" were each measured bypassable by a classifier-directed note in whichever window decided; there is no window a caller can shape that is safe to make the adjudicator. The per-turn context windows and the assistant-turn UNCERTAIN floor (which changed no outcome downstream and cost extra calls) are gone with them, which also removes the short-conversation eviction regression the verifier measured (every per-turn window shifted). Live counts on the benchmark are reported in the PR. The budget/breaker trip merge is now pinned by a test. Public text that quoted the exact wording of a closed injection now describes the class.
